...
/// Nested struct: ListPartitionReassignmentsTopics
...
pub const ListPartitionReassignmentsRequest = struct {
    ...
    /// Create a default instance of ListPartitionReassignmentsRequest
    pub fn default() Self {
        return .{
            .timeout_ms = 60000,
            .topics = null,
    ...
    /// The topics to list partition reassignments for, or null to list everything.
    pub fn withTopics(self: Self, value: ?[]ListPartitionReassignmentsTopics) Self {
        var result = self;
        result.topics = value;
        return result;
    }

    /// Encode ListPartitionReassignmentsRequest
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.topics) |arr| {
                    for (arr) |*item| {
                        try ListPartitionReassignmentsTopics.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of ListPartitionReassignmentsRequest for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.topics) |arr| {
                ...
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(len) else 4;
                for (arr) |*item| {
                    total_size += try ListPartitionReassignmentsTopics.computeSize(item, version);
                }
            } else {
    ...
    /// Decode ListPartitionReassignmentsRequest
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            ...
            if (raw_len < 0) {
                ...
                const array = try allocator.alloc(ListPartitionReassignmentsTopics, array_len);
                for (array) |*item| {
                    item.* = try ListPartitionReassignmentsTopics.decode(reader, version, allocator);
                }
                self.topics = array;
            }
        }

...
/// Nested struct: AlterUserScramCredentialsResult
...
pub const AlterUserScramCredentialsResponse = struct {
    ...
    results: ?[]AlterUserScramCredentialsResult = null,
    ...
    /// Create a default instance of AlterUserScramCredentialsResponse
    pub fn default() Self {
        return .{
            .throttle_time_ms = 0,
            .results = null,
    ...
    /// The results for deletions and alterations, one per affected user.
    pub fn withResults(self: Self, value: ?[]AlterUserScramCredentialsResult) Self {
        var result = self;
        result.results = value;
        return result;
    }

    /// Encode AlterUserScramCredentialsResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.results) |arr| {
                    for (arr) |*item| {
                        try AlterUserScramCredentialsResult.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of AlterUserScramCredentialsResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.results) |arr| {
                ...
                for (arr) |*item| {
                    total_size += try AlterUserScramCredentialsResult.computeSize(item, version);
                }
            } else {
    ...
    /// Decode AlterUserScramCredentialsResponse
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            ...
            const array = try allocator.alloc(AlterUserScramCredentialsResult, array_len);
            for (array) |*item| {
                item.* = try AlterUserScramCredentialsResult.decode(reader, version, allocator);
            }
            self.results = array;
        }

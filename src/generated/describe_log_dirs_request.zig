...
/// Nested struct: DescribableLogDirTopic
...
pub const DescribeLogDirsRequest = struct {
    ...
    topics: ?[]DescribableLogDirTopic = null,
    ...
    /// Create a default instance of DescribeLogDirsRequest
    pub fn default() Self {
        return .{
            .topics = null,
            ._tagged_fields = null,
    ...
    /// Each topic that we want to describe log directories for, or null for all topics.
    pub fn withTopics(self: Self, value: ?[]DescribableLogDirTopic) Self {
        var result = self;
        result.topics = value;
        return result;
    }

    /// Encode DescribeLogDirsRequest
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.topics) |arr| {
                    for (arr) |*item| {
                        try DescribableLogDirTopic.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of DescribeLogDirsRequest for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.topics) |arr| {
                ...
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(len) else 4;
                for (arr) |*item| {
                    total_size += try DescribableLogDirTopic.computeSize(item, version);
                }
            } else {
    ...
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        ...
        if (version >= 0 and version <= 32767) {
            ...
            if (raw_len < 0) {
                ...
                const array = try allocator.alloc(DescribableLogDirTopic, array_len);
                for (array) |*item| {
                    item.* = try DescribableLogDirTopic.decode(reader, version, allocator);
                }
                self.topics = array;
            }
        }

...
/// Nested struct: ConfigResource
pub const ConfigResource = struct {
    const Self = @This();
...
pub const ListConfigResourcesResponse = struct {
    const Self = @This();
    ...
    config_resources: ?[]ConfigResource = null,
    ...
    /// Create a default instance of ListConfigResourcesResponse
    pub fn default() Self {
        return .{
            .throttle_time_ms = 0,
            .error_code = 0,
            .config_resources = null,
            ._tagged_fields = null,
        };
    }
    ...
    /// Each config resource in the response.
    pub fn withConfigResources(self: Self, value: ?[]ConfigResource) Self {
        var result = self;
        result.config_resources = value;
        return result;
    }

    /// Encode ListConfigResourcesResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        // Field: ConfigResources
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                try types.encodeCompactArrayLenNonNull(writer, self.config_resources);
                if (self.config_resources) |arr| {
                    for (arr) |*item| {
                        try ConfigResource.encode(item, writer, version);
                    }
                }
            } else {
                try types.encodeArrayLenNonNull(writer, self.config_resources);
                if (self.config_resources) |arr| {
                    for (arr) |*item| {
                        try ConfigResource.encode(item, writer, version);
                    }
    ...
    /// Compute the size of ListConfigResourcesResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.config_resources) |arr| {
                ...
                for (arr) |*item| {
                    total_size += try ConfigResource.computeSize(item, version);
                }
            } else {
    ...
    /// Decode ListConfigResourcesResponse
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            ...
            const array = try allocator.alloc(ConfigResource, array_len);
            for (array) |*item| {
                item.* = try ConfigResource.decode(reader, version, allocator);
            }
            self.config_resources = array;
        }

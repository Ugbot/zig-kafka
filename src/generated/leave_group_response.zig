...
pub const LeaveGroupResponse = struct {
    ...
    /// Create a default instance of LeaveGroupResponse
    pub fn default() Self {
        return .{
            .throttle_time_ms = 0,
            .error_code = 0,
    ...
    /// List of leaving member responses.
    /// Versions: 3+
    pub fn withMembers(self: Self, value: ?[]MemberResponse) Self {
        var result = self;
        result.members = value;
        return result;
    }

    /// Encode LeaveGroupResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 3 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.members) |arr| {
                    for (arr) |*item| {
                        try MemberResponse.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of LeaveGroupResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 3 and version <= 32767) {
            if (self.members) |arr| {
                ...
                for (arr) |*item| {
                    total_size += try MemberResponse.computeSize(item, version);
                }
            } else {
    ...
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        ...
        if (version >= 3 and version <= 32767) {
            ...
            const array = try allocator.alloc(MemberResponse, array_len);
            for (array) |*item| {
                item.* = try MemberResponse.decode(reader, version, allocator);
            }
            self.members = array;
        }

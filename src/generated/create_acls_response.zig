...
/// Nested struct: AclCreationResult
...
pub const CreateAclsResponse = struct {
    ...
    results: ?[]AclCreationResult = null,
    ...
    /// Create a default instance of CreateAclsResponse
    pub fn default() Self {
        return .{
            .throttle_time_ms = 0,
            .results = null,
    ...
    /// The results for each ACL creation.
    pub fn withResults(self: Self, value: ?[]AclCreationResult) Self {
        var result = self;
        result.results = value;
        return result;
    }

    /// Encode CreateAclsResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.results) |arr| {
                    for (arr) |*item| {
                        try AclCreationResult.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of CreateAclsResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.results) |arr| {
                ...
                for (arr) |*item| {
                    total_size += try AclCreationResult.computeSize(item, version);
                }
            } else {
    ...
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        ...
        if (version >= 0 and version <= 32767) {
            ...
            const array = try allocator.alloc(AclCreationResult, array_len);
            for (array) |*item| {
                item.* = try AclCreationResult.decode(reader, version, allocator);
            }
            self.results = array;
        }

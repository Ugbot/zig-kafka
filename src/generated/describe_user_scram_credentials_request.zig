...
pub const DescribeUserScramCredentialsRequest = struct {
    ...
    /// Create a default instance of DescribeUserScramCredentialsRequest
    pub fn default() Self {
        return .{
            .users = null,
            ._tagged_fields = null,
    ...
    /// The users to describe, or null/empty to describe all users.
    pub fn withUsers(self: Self, value: ?[]UserName) Self {
        var result = self;
        result.users = value;
        return result;
    }

    /// Encode DescribeUserScramCredentialsRequest
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                try types.encodeCompactArrayLen(writer, self.users);
                if (self.users) |arr| {
                    for (arr) |*item| {
                        try UserName.encode(item, writer, version);
                    }
                }
            } else {
                try types.encodeArrayLen(writer, self.users);
                if (self.users) |arr| {
                    for (arr) |*item| {
                        try UserName.encode(item, writer, version);
                    }
    ...
    /// Compute the size of DescribeUserScramCredentialsRequest for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.users) |arr| {
                ...
                for (arr) |*item| {
                    total_size += try UserName.computeSize(item, version);
                }
            } else {
    ...
    /// Decode DescribeUserScramCredentialsRequest
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            ...
            if (raw_len < 0) {
                ...
                const array = try allocator.alloc(UserName, array_len);
                for (array) |*item| {
                    item.* = try UserName.decode(reader, version, allocator);
                }
                self.users = array;
            }
        }

...
pub const DeleteTopicsRequest = struct {
    ...
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 5) {
            if (is_flexible) {
                try types.encodeCompactArrayNonNull([]const u8, writer, self.topic_names, types.encodeCompactString);
            } else {
                try types.encodeArrayNonNull([]const u8, writer, self.topic_names, types.encodeCompactString);
            }
        }

...
/// Nested struct: TxnOffsetCommitResponseTopic
pub const TxnOffsetCommitResponseTopic = struct {
    ...
    /// Versions: 0+
    partitions: ?[]TxnOffsetCommitResponsePartition = null,
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.partitions) |arr| {
                    for (arr) |*item| {
                        try TxnOffsetCommitResponsePartition.encode(item, writer, version);
                    }
                }
            } else {
    ...
    pub fn computeSize(self: *const Self, version: i16) !usize {
        ...
        if (version >= 0 and version <= 32767) {
            if (self.partitions) |arr| {
                ...
                for (arr) |*item| {
                    total_size += try TxnOffsetCommitResponsePartition.computeSize(item, version);
                }
            } else {
    ...
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        ...
        if (version >= 0 and version <= 32767) {
            ...
            const array = try allocator.alloc(TxnOffsetCommitResponsePartition, array_len);
            for (array) |*item| {
                item.* = try TxnOffsetCommitResponsePartition.decode(reader, version, allocator);
            }
...
/// Nested struct: TxnOffsetCommitResponsePartition
...
pub const TxnOffsetCommitResponse = struct {
    ...
    topics: ?[]TxnOffsetCommitResponseTopic = null,
    ...
    /// Create a default instance of TxnOffsetCommitResponse
    pub fn default() Self {
        return .{
            .throttle_time_ms = 0,
            .topics = null,
    ...
    /// The responses for each topic.
    pub fn withTopics(self: Self, value: ?[]TxnOffsetCommitResponseTopic) Self {
        var result = self;
        result.topics = value;
        return result;
    }

    /// Encode TxnOffsetCommitResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.topics) |arr| {
                    for (arr) |*item| {
                        try TxnOffsetCommitResponseTopic.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of TxnOffsetCommitResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.topics) |arr| {
                ...
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(len) else 4;
                for (arr) |*item| {
                    total_size += try TxnOffsetCommitResponseTopic.computeSize(item, version);
                }
            } else {
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(1) else 4;
            }
    ...
    /// Decode TxnOffsetCommitResponse
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            const array_len = if (is_flexible) try types.decodeCompactArrayLen(reader) else try types.decodeArrayLen(reader);
            const array = try allocator.alloc(TxnOffsetCommitResponseTopic, array_len);
            for (array) |*item| {
                item.* = try TxnOffsetCommitResponseTopic.decode(reader, version, allocator);
            }
            self.topics = array;
        }

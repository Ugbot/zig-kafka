...
pub const TopicResponse = struct {
    ...
    /// Versions: 0+
    partitions: ?[]PartitionResponse = null,
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.partitions) |arr| {
                    for (arr) |*item| {
                        try PartitionResponse.encode(item, writer, version);
                    }
                }
            } else {
    ...
    pub fn computeSize(self: *const Self, version: i16) !usize {
        ...
        if (version >= 0 and version <= 32767) {
            if (self.partitions) |arr| {
                ...
                for (arr) |*item| {
                    total_size += try PartitionResponse.computeSize(item, version);
                }
            } else {
    ...
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        ...
        if (version >= 0 and version <= 32767) {
            ...
            const array = try allocator.alloc(PartitionResponse, array_len);
            for (array) |*item| {
                item.* = try PartitionResponse.decode(reader, version, allocator);
            }
...
/// Nested struct: PartitionResponse
pub const PartitionResponse = struct {
    ...
    /// Versions: 0+
    active_producers: ?[]ProducerState = null,
    ...
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        // Field: ActiveProducers
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                try types.encodeCompactArrayLenNonNull(writer, self.active_producers);
                if (self.active_producers) |arr| {
                    for (arr) |*item| {
                        try ProducerState.encode(item, writer, version);
                    }
                }
            } else {
                try types.encodeArrayLenNonNull(writer, self.active_producers);
                if (self.active_producers) |arr| {
                    for (arr) |*item| {
                        try ProducerState.encode(item, writer, version);
                    }
    ...
    pub fn computeSize(self: *const Self, version: i16) !usize {
        ...
        if (version >= 0 and version <= 32767) {
            if (self.active_producers) |arr| {
                ...
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(len) else 4;
                for (arr) |*item| {
                    total_size += try ProducerState.computeSize(item, version);
                }
            } else {
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(1) else 4;
            }
    ...
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        ...
        if (version >= 0 and version <= 32767) {
            const array_len = if (is_flexible) try types.decodeCompactArrayLen(reader) else try types.decodeArrayLen(reader);
            const array = try allocator.alloc(ProducerState, array_len);
            for (array) |*item| {
                item.* = try ProducerState.decode(reader, version, allocator);
            }
            self.active_producers = array;
        }
...
pub const ProducerState = struct {
    ...
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        // Field: ProducerEpoch
        if (version >= 0 and version <= 32767) {
            try types.encodeInt32(writer, self.producer_epoch);
        }

        // Field: LastSequence
        if (version >= 0 and version <= 32767) {
            try types.encodeInt32(writer, self.last_sequence);
        }

        // Field: LastTimestamp
        if (version >= 0 and version <= 32767) {
            try types.encodeInt64(writer, self.last_timestamp);
        }
        ...
        // Field: CurrentTxnStartOffset
        if (version >= 0 and version <= 32767) {
            try types.encodeInt64(writer, self.current_txn_start_offset);
        }
    ...
    pub fn computeSize(self: *const Self, version: i16) !usize {
        ...
        // Field: ProducerEpoch
        if (version >= 0 and version <= 32767) {
            total_size += types.computeSizeInt32(self.producer_epoch);
        }

        // Field: LastSequence
        if (version >= 0 and version <= 32767) {
            total_size += types.computeSizeInt32(self.last_sequence);
        }

        // Field: LastTimestamp
        if (version >= 0 and version <= 32767) {
            total_size += types.computeSizeInt64(self.last_timestamp);
        }
        ...
        // Field: CurrentTxnStartOffset
        if (version >= 0 and version <= 32767) {
            total_size += types.computeSizeInt64(self.current_txn_start_offset);
        }
    ...
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        ...
        // Field: ProducerEpoch
        if (version >= 0 and version <= 32767) {
            self.producer_epoch = try types.decodeInt32(reader);
        }

        // Field: LastSequence
        if (version >= 0 and version <= 32767) {
            self.last_sequence = try types.decodeInt32(reader);
        }

        // Field: LastTimestamp
        if (version >= 0 and version <= 32767) {
            self.last_timestamp = try types.decodeInt64(reader);
        }
        ...
        // Field: CurrentTxnStartOffset
        if (version >= 0 and version <= 32767) {
            self.current_txn_start_offset = try types.decodeInt64(reader);
        }
...
pub const DescribeProducersResponse = struct {
    ...
    /// Create a default instance of DescribeProducersResponse
    pub fn default() Self {
        return .{
            .throttle_time_ms = 0,
            .topics = null,
    ...
    /// Each topic in the response.
    pub fn withTopics(self: Self, value: ?[]TopicResponse) Self {
        var result = self;
        result.topics = value;
        return result;
    }

    /// Encode DescribeProducersResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.topics) |arr| {
                    for (arr) |*item| {
                        try TopicResponse.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of DescribeProducersResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.topics) |arr| {
                ...
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(len) else 4;
                for (arr) |*item| {
                    total_size += try TopicResponse.computeSize(item, version);
                }
            } else {
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(1) else 4;
            }
    ...
    /// Decode DescribeProducersResponse
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            const array_len = if (is_flexible) try types.decodeCompactArrayLen(reader) else try types.decodeArrayLen(reader);
            const array = try allocator.alloc(TopicResponse, array_len);
            for (array) |*item| {
                item.* = try TopicResponse.decode(reader, version, allocator);
            }
            self.topics = array;
        }

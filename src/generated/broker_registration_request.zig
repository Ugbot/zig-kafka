...
pub const BrokerRegistrationRequest = struct {
    ...
    /// Create a default instance of BrokerRegistrationRequest
    pub fn default() Self {
        return .{
            .broker_id = 0,
            .cluster_id = "",
    ...
    /// The rack which this broker is in.
    pub fn withRack(self: Self, value: ?[]const u8) Self {
        var result = self;
        result.rack = value;
        return result;
    }
    ...
    /// If the required configurations for ZK migration are present, this value is set to true.
    /// Versions: 1+
    pub fn withIsMigratingZkBroker(self: Self, value: bool) Self {
        var result = self;
        result.is_migrating_zk_broker = value;
        return result;
    }
    ...
    /// Log directories configured in this broker which are available.
    /// Versions: 2+
    pub fn withLogDirs(self: Self, value: ?[][16]u8) Self {
        var result = self;
        result.log_dirs = value;
        return result;
    }
    ...
    /// The epoch before a clean shutdown.
    /// Versions: 3+
    pub fn withPreviousBrokerEpoch(self: Self, value: i64) Self {
        var result = self;
        result.previous_broker_epoch = value;
        return result;
    }

    /// Encode BrokerRegistrationRequest
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 1 and version <= 32767) {
            try types.encodeBoolean(writer, self.is_migrating_zk_broker);
        }

        // Field: LogDirs
        if (version >= 2 and version <= 32767) {
            if (is_flexible) {
                try types.encodeCompactArrayNonNull([16]u8, writer, self.log_dirs, types.encodeUuid);
            } else {
                try types.encodeArrayNonNull([16]u8, writer, self.log_dirs, types.encodeUuid);
            }
        }

        // Field: PreviousBrokerEpoch
        if (version >= 3 and version <= 32767) {
            try types.encodeInt64(writer, self.previous_broker_epoch);
        }
    ...
    /// Compute the size of BrokerRegistrationRequest for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        // Field: IsMigratingZkBroker
        if (version >= 1 and version <= 32767) {
            total_size += types.computeSizeBoolean(self.is_migrating_zk_broker);
        }

        // Field: LogDirs
        if (version >= 2 and version <= 32767) {
            if (self.log_dirs) |arr| {
                const len: u32 = @intCast(arr.len + 1);
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(len) else 4;
                for (arr) |item| {
                    total_size += types.computeSizeUuid(item);
                }
            } else {
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(1) else 4;
            }
        ...
        // Field: PreviousBrokerEpoch
        if (version >= 3 and version <= 32767) {
            total_size += types.computeSizeInt64(self.previous_broker_epoch);
        }
    ...
    /// Decode BrokerRegistrationRequest
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        // Field: IsMigratingZkBroker
        if (version >= 1 and version <= 32767) {
            self.is_migrating_zk_broker = try types.decodeBoolean(reader);
        }

        // Field: LogDirs
        if (version >= 2 and version <= 32767) {
            self.log_dirs = if (is_flexible)
                try types.decodeCompactPrimitiveArray([16]u8, reader, allocator, types.decodeUuid)
            else
                try types.decodePrimitiveArray([16]u8, reader, allocator, types.decodeUuid);
        }

        // Field: PreviousBrokerEpoch
        if (version >= 3 and version <= 32767) {
            self.previous_broker_epoch = try types.decodeInt64(reader);
        }

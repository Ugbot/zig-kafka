...
/// Nested struct: ShareAcknowledgeTopicResponse
...
pub const ShareAcknowledgeResponse = struct {
    ...
    error_message: ?[]const u8 = null,
    /// The response topics.
    responses: ?[]ShareAcknowledgeTopicResponse = null,
    ...
    /// Create a default instance of ShareAcknowledgeResponse
    pub fn default() Self {
        return .{
            .throttle_time_ms = 0,
            .error_code = 0,
    ...
    /// The response topics.
    pub fn withResponses(self: Self, value: ?[]ShareAcknowledgeTopicResponse) Self {
        var result = self;
    ...
    /// Encode ShareAcknowledgeResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.responses) |arr| {
                    for (arr) |*item| {
                        try ShareAcknowledgeTopicResponse.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of ShareAcknowledgeResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.responses) |arr| {
                ...
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(len) else 4;
                for (arr) |*item| {
                    total_size += try ShareAcknowledgeTopicResponse.computeSize(item, version);
                }
            } else {
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(1) else 4;
            }
    ...
    /// Decode ShareAcknowledgeResponse
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            const array_len = if (is_flexible) try types.decodeCompactArrayLen(reader) else try types.decodeArrayLen(reader);
            const array = try allocator.alloc(ShareAcknowledgeTopicResponse, array_len);
            for (array) |*item| {
                item.* = try ShareAcknowledgeTopicResponse.decode(reader, version, allocator);
            }

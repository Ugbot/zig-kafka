...
/// Nested struct: WriteStateResult
pub const WriteStateResult = struct {
    const Self = @This();
...
pub const WriteShareGroupStateResponse = struct {
    const Self = @This();

    /// The write results.
    results: ?[]WriteStateResult = null,
    ...
    /// Create a default instance of WriteShareGroupStateResponse
    pub fn default() Self {
        return .{
            .results = null,
            ._tagged_fields = null,
    ...
    /// The write results.
    pub fn withResults(self: Self, value: ?[]WriteStateResult) Self {
        var result = self;
        result.results = value;
        return result;
    }

    /// Encode WriteShareGroupStateResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.results) |arr| {
                    for (arr) |*item| {
                        try WriteStateResult.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of WriteShareGroupStateResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.results) |arr| {
                ...
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(len) else 4;
                for (arr) |*item| {
                    total_size += try WriteStateResult.computeSize(item, version);
                }
            } else {
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(1) else 4;
            }
    ...
    /// Decode WriteShareGroupStateResponse
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            const array_len = if (is_flexible) try types.decodeCompactArrayLen(reader) else try types.decodeArrayLen(reader);
            const array = try allocator.alloc(WriteStateResult, array_len);
            for (array) |*item| {
                item.* = try WriteStateResult.decode(reader, version, allocator);
            }
            self.results = array;
        }

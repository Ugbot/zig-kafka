...
/// Nested struct: ReadStateSummaryResult
...
pub const ReadShareGroupStateSummaryResponse = struct {
    ...
    /// The read results.
    results: ?[]ReadStateSummaryResult = null,
    ...
    /// Create a default instance of ReadShareGroupStateSummaryResponse
    pub fn default() Self {
        return .{
            .results = null,
            ._tagged_fields = null,
    ...
    /// The read results.
    pub fn withResults(self: Self, value: ?[]ReadStateSummaryResult) Self {
        var result = self;
        result.results = value;
        return result;
    }

    /// Encode ReadShareGroupStateSummaryResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.results) |arr| {
                    for (arr) |*item| {
                        try ReadStateSummaryResult.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of ReadShareGroupStateSummaryResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.results) |arr| {
                ...
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(len) else 4;
                for (arr) |*item| {
                    total_size += try ReadStateSummaryResult.computeSize(item, version);
                }
            } else {
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(1) else 4;
            }
    ...
    /// Decode ReadShareGroupStateSummaryResponse
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            const array_len = if (is_flexible) try types.decodeCompactArrayLen(reader) else try types.decodeArrayLen(reader);
            const array = try allocator.alloc(ReadStateSummaryResult, array_len);
            for (array) |*item| {
                item.* = try ReadStateSummaryResult.decode(reader, version, allocator);
            }
            self.results = array;
        }

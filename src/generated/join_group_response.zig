...
/// Nested struct: JoinGroupResponseMember
...
pub const JoinGroupResponse = struct {
    ...
    member_id: []const u8 = "",
    /// The group members.
    members: ?[]JoinGroupResponseMember = null,
    ...
    /// Create a default instance of JoinGroupResponse
    pub fn default() Self {
        return .{
            .throttle_time_ms = 0,
            .error_code = 0,
    ...
    /// The leader of the group.
    pub fn withLeader(self: Self, value: []const u8) Self {
        var result = self;
        result.leader = value;
        return result;
    }
    ...
    /// True if the leader must skip running the assignment.
    /// Versions: 9+
    pub fn withSkipAssignment(self: Self, value: bool) Self {
        var result = self;
        result.skip_assignment = value;
        return result;
    }
    ...
    /// The group members.
    pub fn withMembers(self: Self, value: ?[]JoinGroupResponseMember) Self {
        var result = self;
        result.members = value;
        return result;
    }

    /// Encode JoinGroupResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                try types.encodeCompactString(writer, self.leader);
            } else {
                try types.encodeString(writer, self.leader);
            }
        }

        // Field: SkipAssignment
        if (version >= 9 and version <= 32767) {
            try types.encodeBoolean(writer, self.skip_assignment);
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.members) |arr| {
                    for (arr) |*item| {
                        try JoinGroupResponseMember.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of JoinGroupResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        // Field: Leader
        if (version >= 0 and version <= 32767) {
            total_size += if (is_flexible) types.computeSizeCompactString(self.leader) else types.computeSizeString(self.leader);
        }

        // Field: SkipAssignment
        if (version >= 9 and version <= 32767) {
            total_size += types.computeSizeBoolean(self.skip_assignment);
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.members) |arr| {
                ...
                for (arr) |*item| {
                    total_size += try JoinGroupResponseMember.computeSize(item, version);
                }
            } else {
    ...
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        ...
        // Field: SkipAssignment
        if (version >= 9 and version <= 32767) {
            self.skip_assignment = try types.decodeBoolean(reader);
        }
        ...
        if (version >= 0 and version <= 32767) {
            ...
            const array = try allocator.alloc(JoinGroupResponseMember, array_len);
            for (array) |*item| {
                item.* = try JoinGroupResponseMember.decode(reader, version, allocator);
            }
            self.members = array;
        }

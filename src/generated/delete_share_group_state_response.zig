...
//! Message: DeleteShareGroupStateResponse
//! API Key: 86
//! Type: response
//! Valid Versions: 0
//! Flexible Versions: 0+
...
/// Nested struct: DeleteStateResult
pub const DeleteStateResult = struct {
    const Self = @This();
...
pub const DeleteShareGroupStateResponse = struct {
    const Self = @This();

    /// The delete results.
    results: ?[]DeleteStateResult = null,
    ...
    /// Create a default instance of DeleteShareGroupStateResponse
    pub fn default() Self {
        return .{
            .results = null,
            ._tagged_fields = null,
    ...
    /// The delete results.
    pub fn withResults(self: Self, value: ?[]DeleteStateResult) Self {
        var result = self;
        result.results = value;
        return result;
    }

    /// Encode DeleteShareGroupStateResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.results) |arr| {
                    for (arr) |*item| {
                        try DeleteStateResult.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of DeleteShareGroupStateResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.results) |arr| {
                ...
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(len) else 4;
                for (arr) |*item| {
                    total_size += try DeleteStateResult.computeSize(item, version);
                }
            } else {
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(1) else 4;
            }
    ...
    /// Decode DeleteShareGroupStateResponse
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            const array_len = if (is_flexible) try types.decodeCompactArrayLen(reader) else try types.decodeArrayLen(reader);
            const array = try allocator.alloc(DeleteStateResult, array_len);
            for (array) |*item| {
                item.* = try DeleteStateResult.decode(reader, version, allocator);
            }
            self.results = array;
        }

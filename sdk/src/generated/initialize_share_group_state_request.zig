...
/// Nested struct: InitializeStateData
...
pub const InitializeShareGroupStateRequest = struct {
    ...
    topics: ?[]InitializeStateData = null,
    ...
    /// Create a default instance of InitializeShareGroupStateRequest
    pub fn default() Self {
        return .{
            .group_id = "",
            .topics = null,
    ...
    /// The data for the topics.
    pub fn withTopics(self: Self, value: ?[]InitializeStateData) Self {
        var result = self;
        result.topics = value;
        return result;
    }

    /// Encode InitializeShareGroupStateRequest
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.topics) |arr| {
                    for (arr) |*item| {
                        try InitializeStateData.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of InitializeShareGroupStateRequest for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.topics) |arr| {
                ...
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(len) else 4;
                for (arr) |*item| {
                    total_size += try InitializeStateData.computeSize(item, version);
                }
            } else {
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(1) else 4;
            }
    ...
    /// Decode InitializeShareGroupStateRequest
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            const array_len = if (is_flexible) try types.decodeCompactArrayLen(reader) else try types.decodeArrayLen(reader);
            const array = try allocator.alloc(InitializeStateData, array_len);
            for (array) |*item| {
                item.* = try InitializeStateData.decode(reader, version, allocator);
            }
            self.topics = array;
        }

...
/// Nested struct: OngoingTopicReassignment
pub const OngoingTopicReassignment = struct {
    ...
    /// Versions: 0+
    partitions: ?[]OngoingPartitionReassignment = null,
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.partitions) |arr| {
                    for (arr) |*item| {
                        try OngoingPartitionReassignment.encode(item, writer, version);
                    }
                }
            } else {
    ...
    pub fn computeSize(self: *const Self, version: i16) !usize {
        ...
        if (version >= 0 and version <= 32767) {
            if (self.partitions) |arr| {
                ...
                for (arr) |*item| {
                    total_size += try OngoingPartitionReassignment.computeSize(item, version);
                }
            } else {
    ...
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        ...
        if (version >= 0 and version <= 32767) {
            ...
            const array = try allocator.alloc(OngoingPartitionReassignment, array_len);
            for (array) |*item| {
                item.* = try OngoingPartitionReassignment.decode(reader, version, allocator);
            }
...
/// Nested struct: OngoingPartitionReassignment
pub const OngoingPartitionReassignment = struct {
    ...
    /// Versions: 0+
    adding_replicas: ?[]i32 = null,
    /// The set of replicas we are currently removing.
    /// Versions: 0+
    removing_replicas: ?[]i32 = null,
    ...
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                try types.encodeCompactArrayNonNull(i32, writer, self.replicas, types.encodeInt32);
            } else {
                try types.encodeArrayNonNull(i32, writer, self.replicas, types.encodeInt32);
            }
        }

        // Field: AddingReplicas
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                try types.encodeCompactArrayNonNull(i32, writer, self.adding_replicas, types.encodeInt32);
            } else {
                try types.encodeArrayNonNull(i32, writer, self.adding_replicas, types.encodeInt32);
            }
        }

        // Field: RemovingReplicas
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                try types.encodeCompactArrayNonNull(i32, writer, self.removing_replicas, types.encodeInt32);
            } else {
                try types.encodeArrayNonNull(i32, writer, self.removing_replicas, types.encodeInt32);
            }
        }
    ...
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        ...
        // Field: AddingReplicas
        if (version >= 0 and version <= 32767) {
            self.adding_replicas = if (is_flexible)
                try types.decodeCompactPrimitiveArray(i32, reader, allocator, types.decodeInt32)
            else
                try types.decodePrimitiveArray(i32, reader, allocator, types.decodeInt32);
        }
        ...
        // Field: RemovingReplicas
        if (version >= 0 and version <= 32767) {
            self.removing_replicas = if (is_flexible)
                try types.decodeCompactPrimitiveArray(i32, reader, allocator, types.decodeInt32)
            else
                try types.decodePrimitiveArray(i32, reader, allocator, types.decodeInt32);
        }
...
pub const ListPartitionReassignmentsResponse = struct {
    ...
    topics: ?[]OngoingTopicReassignment = null,
    ...
    /// Create a default instance of ListPartitionReassignmentsResponse
    pub fn default() Self {
        return .{
            .throttle_time_ms = 0,
            .error_code = 0,
    ...
    /// The ongoing reassignments for each topic.
    pub fn withTopics(self: Self, value: ?[]OngoingTopicReassignment) Self {
        var result = self;
        result.topics = value;
        return result;
    }

    /// Encode ListPartitionReassignmentsResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.topics) |arr| {
                    for (arr) |*item| {
                        try OngoingTopicReassignment.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of ListPartitionReassignmentsResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.topics) |arr| {
                ...
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(len) else 4;
                for (arr) |*item| {
                    total_size += try OngoingTopicReassignment.computeSize(item, version);
                }
            } else {
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(1) else 4;
            }
    ...
    /// Decode ListPartitionReassignmentsResponse
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            const array_len = if (is_flexible) try types.decodeCompactArrayLen(reader) else try types.decodeArrayLen(reader);
            const array = try allocator.alloc(OngoingTopicReassignment, array_len);
            for (array) |*item| {
                item.* = try OngoingTopicReassignment.decode(reader, version, allocator);
            }
            self.topics = array;
        }

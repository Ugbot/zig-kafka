...
/// Nested struct: DescribeAclsResource
pub const DescribeAclsResource = struct {
    ...
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                try types.encodeCompactArrayLenNonNull(writer, self.acls);
                if (self.acls) |arr| {
                    for (arr) |*item| {
                        try AclDescription.encode(item, writer, version);
                    }
                }
            } else {
                try types.encodeArrayLenNonNull(writer, self.acls);
                if (self.acls) |arr| {
                    for (arr) |*item| {
                        try AclDescription.encode(item, writer, version);
                    }
    ...
    pub fn computeSize(self: *const Self, version: i16) !usize {
        ...
        if (version >= 0 and version <= 32767) {
            if (self.acls) |arr| {
                ...
                for (arr) |*item| {
                    total_size += try AclDescription.computeSize(item, version);
                }
            } else {
    ...
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        ...
        if (version >= 0 and version <= 32767) {
            ...
            const array = try allocator.alloc(AclDescription, array_len);
            for (array) |*item| {
                item.* = try AclDescription.decode(reader, version, allocator);
            }
            self.acls = array;
        }
...
pub const DescribeAclsResponse = struct {
    ...
    resources: ?[]DescribeAclsResource = null,
    ...
    /// Create a default instance of DescribeAclsResponse
    pub fn default() Self {
        return .{
            .throttle_time_ms = 0,
            .error_code = 0,
    ...
    /// Each Resource that is referenced in an ACL.
    pub fn withResources(self: Self, value: ?[]DescribeAclsResource) Self {
        var result = self;
        result.resources = value;
        return result;
    }

    /// Encode DescribeAclsResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.resources) |arr| {
                    for (arr) |*item| {
                        try DescribeAclsResource.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of DescribeAclsResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.resources) |arr| {
                ...
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(len) else 4;
                for (arr) |*item| {
                    total_size += try DescribeAclsResource.computeSize(item, version);
                }
            } else {
                total_size += if (is_flexible) types.computeSizeUnsignedVarInt(1) else 4;
            }
    ...
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        ...
        if (version >= 0 and version <= 32767) {
            const array_len = if (is_flexible) try types.decodeCompactArrayLen(reader) else try types.decodeArrayLen(reader);
            const array = try allocator.alloc(DescribeAclsResource, array_len);
            for (array) |*item| {
                item.* = try DescribeAclsResource.decode(reader, version, allocator);
            }

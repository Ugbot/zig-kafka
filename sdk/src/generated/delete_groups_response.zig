...
/// Nested struct: DeletableGroupResult
...
pub const DeleteGroupsResponse = struct {
    ...
    results: ?[]DeletableGroupResult = null,
    ...
    /// Create a default instance of DeleteGroupsResponse
    pub fn default() Self {
        return .{
            .throttle_time_ms = 0,
            .results = null,
    ...
    /// The deletion results.
    pub fn withResults(self: Self, value: ?[]DeletableGroupResult) Self {
        var result = self;
        result.results = value;
        return result;
    }

    /// Encode DeleteGroupsResponse
    pub fn encode(self: *const Self, writer: anytype, version: i16) !void {
        ...
        if (version >= 0 and version <= 32767) {
            if (is_flexible) {
                ...
                if (self.results) |arr| {
                    for (arr) |*item| {
                        try DeletableGroupResult.encode(item, writer, version);
                    }
                }
            } else {
    ...
    /// Compute the size of DeleteGroupsResponse for the given version
    pub fn computeSize(self: *const Self, version: i16) !usize {
        if (!isValidVersion(version)) {
            return types.Error.UnsupportedVersion;
        }
        ...
        if (version >= 0 and version <= 32767) {
            if (self.results) |arr| {
                ...
                for (arr) |*item| {
                    total_size += try DeletableGroupResult.computeSize(item, version);
                }
            } else {
    ...
    pub fn decode(reader: anytype, version: i16, allocator: std.mem.Allocator) !Self {
        ...
        if (version >= 0 and version <= 32767) {
            ...
            const array = try allocator.alloc(DeletableGroupResult, array_len);
            for (array) |*item| {
                item.* = try DeletableGroupResult.decode(reader, version, allocator);
            }
            self.results = array;
        }
